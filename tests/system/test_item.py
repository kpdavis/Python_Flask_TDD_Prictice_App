...
class ItemTest(BaseTest):
    ...
    def test_get_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'name': 'test', 'price': 10.99}, json.loads(resp.data))
    ...
    def test_create_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                resp = client.post('/item/test', data={'price': 10.99, 'store_id': 1})
    ...
    def test_put_item(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                resp = client.put('/item/test', data={'price': 10.99, 'store_id': 1})
    ...
    def test_put_update_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertDictEqual({'name': 'test', 'price': 1.99}, json.loads(resp.data))
    ...
    def test_item_list(self):
        with self.app() as client:
            with self.app_context():
                ...
                self.assertEqual(resp.status_code, 200)
                self.assertDictEqual({'items': [{'name': 'test', 'price': 10.99}]}, json.loads(resp.data))
